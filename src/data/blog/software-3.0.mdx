...
<p id="footnote-5" className="text-sm text-gray-500 mt-2 italic">⁵ Describing LLMs as "next-word predictors" is a massive oversimplification. Since the GPT-3 paper, the training pipeline of LLMs has evolved. While there is still a "next-word prediction" training part (called "pre-training"), most of resources dedicated to training are now spent on later steps (call "post-training") that involve Reinforcement Learning on Human Feedback (RLHF) and Reinforcement Learning with Verifiable Reward (RLVR). These techniques are not as well documented for now and definitely deserve there own blog post 😉.</p>
