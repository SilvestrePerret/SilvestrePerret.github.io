    ...
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border border-border bg-muted/20">
        <p className="text-foreground/60">
          No data to display. Play some games!
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/20 p-4">
      ...
      <div className="relative h-64">
        {/* Y-axis label */}
        <div className="absolute top-1/2 -left-2 -translate-y-1/2 -rotate-90 text-xs text-foreground/60">
          {yLabel}
        </div>

        {/* Bars */}
        <div className="flex h-full items-end justify-around gap-1 pr-2 pl-8">
          {data.bins.map((bin, i) => {
            ...
            return (
              ...
              >
                {/* Bar */}
                <div
                  className="w-full rounded-t bg-accent transition-all duration-300 hover:opacity-80"
                  style={{ height: `${heightPx}px` }}
                  title={`${rangeLabel}: ${count} games (${percentage.toFixed(1)}%)`}
                >
                  {/* Tooltip on hover */}
                  {count > 0 && (
                    <div className="invisible absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 rounded bg-foreground px-2 py-1 text-xs whitespace-nowrap text-background opacity-0 transition-opacity group-hover:visible group-hover:opacity-100">
                      <div className="font-semibold">{rangeLabel}</div>
                      <div>
                        {count} games ({percentage.toFixed(1)}%)
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* X-axis labels */}
      <div className="mt-2 flex justify-around pr-2 pl-8">
        {data.bins.map((bin, i) => (
          <div
            key={i}
            className="flex-1 text-center text-xs text-foreground/60"
          >
