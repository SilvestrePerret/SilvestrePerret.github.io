...
export default function CoinFlipGameB() {
  ...
  const expectedValue =
    coinFairness >= 0.5 ? Infinity : STARTING_AMOUNT / (1 - 2 * coinFairness);
    ...
    <div className="my-8 space-y-6 rounded-xl border-2 border-accent/30 bg-background p-6 shadow-lg">
      <div>
        ...
        <p className="text-foreground/80">
          Start with ${STARTING_AMOUNT}. Each heads doubles your money. First
          tails, you stop.
        </p>
      </div>
      ...
      {coinFairness >= 0.5 && results.length > 0 && (
        <div className="rounded-lg border border-accent/50 bg-accent/10 p-4">
          <p className="text-sm">
            <strong>Note:</strong> With coin fairness ≥ 0.5, the expected value
            is infinite! However, in practice, most games end quickly with small
            rewards. This is a power law distribution.
          </p>
        </div>
      )}
    </div>
